...
const staticAsset = new Asset(stack, 'ApiDefinitionAsset', {
  path: path.join(__dirname, 'openapi/apiDefinition.yaml')
});

const fromBucketApiDefinition = ApiDefinition.fromBucket(staticAsset.bucket, staticAsset.s3ObjectKey);

new OpenApiGatewayToLambda(stack, 'OpenApiGatewayToLambda', {
  apiDefinition: fromBucketApiDefinition,
  apiIntegrations: [
    {
      id: 'MessagesHandler',
      lambdaFunctionProps: {
